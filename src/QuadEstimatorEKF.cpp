...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(3) = predictedState(3) + accelInWorldFrame.x * dt;
  predictedState(4) = predictedState(4) + accelInWorldFrame.y * dt;
  predictedState(5) = predictedState(5) + accelInWorldFrame.z * dt - (float)CONST_GRAVITY * dt;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
	  ...
	  RbgPrime << -cos(roll) * sin(yaw), -sin(pitch) * sin(roll) * sin(yaw) - cos(pitch) * cos(yaw), -cos(pitch) * sin(roll) * sin(yaw) + sin(pitch) * cos(yaw),
		  cos(roll) * cos(yaw), sin(pitch) * sin(roll) * cos(yaw) - cos(pitch) * sin(yaw), cos(pitch) * sin(roll) * cos(yaw) + sin(pitch) * sin(yaw),
		  0, 0, 0;
